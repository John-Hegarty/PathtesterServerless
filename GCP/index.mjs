...
async function  logResult(resolve, record_id, uri ,  testResult , httpcode , errorcode,  time, db )
{
    ...
    await query(`INSERT INTO results (test_id, url,result,httpcode,errorcode,time) VALUES  ( ${record_id},'${uri}','${testResult}',${httpcode},'${errorcode}',${ Math.floor(time)});`, db);
...
if(testPromises.length == 0)
{
 ...
 Promise.allSettled(testPromises).then(([result]) => {
    db.end();
